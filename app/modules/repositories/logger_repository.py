...
class LoggerRepository(BaseRepository):
    ...
    async def login_logger(self, ip: str):
        async with ConnectPG().get_session() as session:
            session.add(
                Logger(logger_ip=ip, create_at=datetime.now().strftime(TIME_FORMAT))
            )
        # self.session.add(
        #     self.model(
        #         logger_ip=ip,
    ...
    async def clear_log(self):
        async with ConnectPG().get_session() as session:
            ...
            clear_loggers = [
                ...
                if idx >= 50 or self.__check_is_outtime_logger(create_at=logger.create_at)
            ]
